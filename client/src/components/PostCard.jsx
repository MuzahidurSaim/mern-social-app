        ...
        <div className='bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl'>
            ...
            <div className='grid grid-cols-2 gap-2'>
                {post.image_urls.map((img, index) => (
                    <img src={img} key={index} alt="post-image" className={`w-full h-48 object-cover rounded-lg ${post.image_urls.length === 1 && 'col-span-2 h-auto'}`} />
                ))}
            </div>
            ...
            <div className='flex items-center gap-4 text-gray-600 text-sm pt-2 border-t border-gray-300'>
                <div className='flex items-center gap-1'>
                    <Heart className={`w-4 h-4 cursor-pointer ${likes.includes(currentUser.id) && 'text-red-500 fill-red-500'}`} onClick={handleLike} />
